"""
BlackMagicAI License Manager — Key Generation & Validation System
...
MASTER_SECRET = os.getenv("BlackMagicAI_MASTER_SECRET", "BlackMagicAI-master-secret-change-me-2026")
...
def generate_key(duration_days: int = 30, customer_id: str = "", price: float = 0.0) -> str:
    ...
    Returns:
        License key string: BlackMagicAI-XXXX-XXXX-XXXX
    """
    ...
    # Format: BlackMagicAI-PPPPPP-SSSSSS (payload + signature)
    ...
    return f"BlackMagicAI-{part1}-{part2}-{part3}"
...
def validate_key(key: str) -> Tuple[bool, str, Optional[int]]:
    ...
    # Normalize: remove spaces, accept with or without BlackMagicAI- prefix
    if not key.startswith("BlackMagicAI-"):
        return False, "❌ ទម្រង់ License Key មិនត្រឹមត្រូវ។ ត្រូវចាប់ផ្តើមដោយ BlackMagicAI-", None

    key_body = key[7:]  # Remove "BlackMagicAI-"
...
def check_license_on_startup():
    ...
    if valid:
        ...
        print(f"   Activate: /activate BlackMagicAI-XXXX-XXXX-XXXX")
    return valid
